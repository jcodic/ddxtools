...
public class Hash implements Progress {
    ...
    public static enum Command {HASH, HASH_STRING, HASH_HEX, CHECK, PRINT};
    ...
    private String getHash() {
        ...
        return Utils.toHex(md.digest()).toLowerCase();
    }
    ...
    private boolean processHashHexString() throws Exception {
        ...
        byte[] bf = Utils.fromHex(settings.sourceString);
        
        Utils.out.println("Hashing hex ["+settings.sourceString+"] length "+bf.length+" bytes, with settings: ");
        printAlgo();
    ...
    public boolean run() throws Exception {
        ...
        switch (command) {
            ...
            case HASH_HEX           : return processHashHexString();
    ...
    private static void printInfo() {
        ...
        Utils.out.println( 5, "hash_hex - same to hash_string but input is hex-string");
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            switch (args[0].toLowerCase()) {
                ...
                case "hash_hex"  : 
                {
                    tool.setCommand(Command.HASH_HEX); 
                    if (!Utils.checkArgs(args, 2)) return;
                    String source = args[1];
                    if (source.equalsIgnoreCase("fromclip")) source = Utils.receiveStringFromClipboard();
                    tool.setSourceString(source);
